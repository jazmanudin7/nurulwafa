...
class Model_santri extends CI_Model
{
	function getDataSantri($rowno, $rowperpage, $nps = "", $nama_santri = "")
	{
		$this->db->select('*');
		$this->db->from('santri');
		$this->db->order_by('nama_santri', 'desc');
	...
	function insert()
	{
		...
		$this->db->select('Right(santri.nps,3) as kode ', false);
		$this->db->where('LEFT(nps,4)', $tahun_masuk);
		$this->db->order_by('nps', 'desc');
		$this->db->limit(10);
