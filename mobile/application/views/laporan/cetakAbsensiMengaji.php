...
$kitab         = $this->db->query("SELECT * FROM kitab WHERE kode_kitab = '$kode_kitab' ")->row_array();
?>
        ...
        <div class="card-body" style="zoom:85%">
            <div class="col-lg-12">
                <h1 class="card-title" style="text-align: center; font-weight: bold;text-transform:uppercase">
                    ...
                    <?php } ?>
                    <?php if ($kode_kitab != '') { ?>
                        KITAB <?php echo $kitab['nama_kitab']; ?><br>
                    <?php } else { ?>
                        SEMUA KITAB<br>
                    <?php } ?>
                        ...
                        ?>
                            <tr>
                                ...
                                while (strtotime($dari) <= strtotime($sampai)) {
                                    ...
                                    santri.nama_santri,
                                    santri.nps,
                                    MAX(IF (absensi_mengaji.tanggal = '$dari' , keterangan , '')) AS keterangan,
                                    COUNT(CASE WHEN keterangan = 'Alfa' AND absensi_mengaji.tanggal = '$dari'  THEN '' END) AS alfa,
                                    COUNT(CASE WHEN keterangan = 'Izin' AND absensi_mengaji.tanggal = '$dari'  THEN '' END) AS izin,
                                    COUNT(CASE WHEN keterangan = 'Hadir' AND absensi_mengaji.tanggal = '$dari'  THEN '' END) AS hadir,
                                    COUNT(CASE WHEN keterangan = 'Sakit' AND absensi_mengaji.tanggal = '$dari'  THEN '' END) AS sakit
                                    FROM santri
                                    LEFT JOIN absensi_mengaji_detail ON absensi_mengaji_detail.nps = santri.nps 
                                    LEFT JOIN absensi_mengaji ON absensi_mengaji.no_absensi = absensi_mengaji_detail.no_absensi 
                                    WHERE absensi_mengaji_detail.nps = '$d->nps' AND absensi_mengaji.tahun_ajaran = '$tahun_ajaran' AND absensi_mengaji.semester = '$semester'
                                    ")->row_array();
